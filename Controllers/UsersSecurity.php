...
if (json_decode(file_get_contents("php://input")) != null) {
    if (!empty(json_decode(file_get_contents("php://input"))->mailAddress) && !empty(json_decode(file_get_contents("php://input"))->oldPassword) && !empty(json_decode(file_get_contents("php://input"))->newPassword) && json_decode(file_get_contents("php://input"))->mailAddress != $_SESSION['User']['mailAddress']) {
    ...
    } else if (!empty(json_decode(file_get_contents("php://input"))->oldPassword) && !empty(json_decode(file_get_contents("php://input"))->newPassword)) {
        $User->changePassword();
    } else {
        $response = array(
            "status" => 1,
